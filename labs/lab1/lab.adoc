...
. Edit the _manifest.yml_ file to give your app a unique name _(only necessary if multiple users are sharing a space)_  Change _workshop_ to _workshop-<yourname>_:
+
----
applications:
- name: workshop-phopper
  memory: 512M
----
...
====
----
Using manifest file /Users/phopper/workspace/DISH-PCF-Workshop-101/cf-spring-mvc-boot/manifest.yml <1>
...
Uploading app files from: /Users/phopper/workspace/DISH-PCF-Workshop-101/cf-spring-mvc-boot/target/cf-spring-mvc-boot-0.0.1-SNAPSHOT.jar
...
Here the droplet is being uploaded to Pivotal Cloudfoundry's internal blobstore so that it can be easily copied to one or more _http://docs.cloudfoundry.org/concepts/architecture/#diego-cell[Diego Cells]_ for execution.
